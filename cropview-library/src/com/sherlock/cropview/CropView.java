...
public class CropView extends View {
	...
	public void initialize(Bitmap image, Rect cropRect, int rotation) {
		mImage = image;
		mRotation = rotation;
		RectF imageRect = new RectF(0, 0, image.getWidth(), image.getHeight());
		mCropObj = new CropObject(imageRect, cropRect, rotation);
		mDirty = true;
		invalidate();
	}
	...
	public void onDraw(Canvas canvas) {
		...
		if (mDisplayImageMatrix == null || mDisplayCropMatrix == null) {
			...
			if (!CropMath.setImageToScreenMatrix(mDisplayImageMatrix,
					mCropObj.getImageRect(), mScreenInCanvas, mRotation)) {
